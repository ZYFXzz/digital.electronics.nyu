...
void lightLedVisualizer() {
  ...
  closeVisualizer();
  int x = map(volume, 0, 1.0 , 0, 255);  // x, a factor for the colors, map the volume to neopixel colors, from 0, 0.5, to blue - green - red
  ...
  neopixel.setPixelColor(1, red, green, blue);
  neopixel.show();
}
...
void checkVolume() {
  ...
  volume = map(volume, 0, 1023, 0.0, 1.0);
}
